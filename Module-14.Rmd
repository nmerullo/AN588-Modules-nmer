...
**This immediately gives us a nice boxplot.** Note that if we’d try to use this command after loading in “gender” as character rather than factor data, R would have thrown an error. Always make sure your variables are in the right format!
...
summary(m)
```
r squared value - our model/height is only explaining 21% of the variance

If we take a look at the summary() of our model, m, we see the same kind of table of results we have seen before, but because the predictor variable in this case is a factor vector instead of a numeric vector, the coefficients are reported and interpreted a bit differently. The coefficient for the intercept, i.e., β0, reflects the estimate of the mean height for the first of our level variables (in this case, Female).
...
summary(m)
```
 Last column of the coefficients shows that female height is significantly different from male, but nonbinary is not
...
levels(z$occupation)
```
This is called binning- putting the 26 different majors into 4 categories
...
p
```
No huge differences in zombie killing abilites. Not significant. 
...
If height different across genders? Null would be the same. Is zombies killed different across occupations?
...
Load in the “gibbon-femurs.csv” dataset, which contains the lengths, in centimeters, of the femora of 400 juvenile, subadult, and adult individuals gibbons. Use both ANOVA and simple linear regession to examine the relationship between age and femur length. Before beginning, make sure that you check for normality of observations within each group.
...
Only care about normality within the age categoies- data can be non normal across all age stages
...
summary(m)
```
R-squared value is showing that age categories are explaining femur length
F statistic is very signficant so there are signficcant differences in femur lengths
Each of those Pr coefficients are just getting compared to infant not to each other.
...
m
```
They are controlling for each other - age controlling for sex and sex controlling for age and they are each significant
...
See module for formula for chi square statistic

We then compare the value of the χ2 statistic to the Chi-Square distribution with k−1 degrees of freedom.
...
The proportion of survivors is different from equal survivorship across all occupations
