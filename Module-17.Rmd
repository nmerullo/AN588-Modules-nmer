...
The logit transformation, then, is the link function connecting Y
 to our predictors. The logit is useful as it converts probabilities, which lie in the range 0 to 1, into the scale of the whole real number line.
...
GRE scores significantly predictive of getting into grad school. The odds of getting into getting into grad school increase by .0035 units for every unit of increase in change in GRE.
...
summary(glmGGR)
```
Each drop in rank means less likelihood of getting admitted and it gets more significant
Lower AIC so this model is better at explaining admittance
...
```
Interaction effects don't matter
