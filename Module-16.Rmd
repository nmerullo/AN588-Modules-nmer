...
add1(m0, scope = . ~ . + age + weight + zombies_killed + years_of_education,
    test = "F")
```
For AIC, the smallest number imples the best model. So height ~ weight is our best model.
...
summary(m2)
```
Height is explained by wegiht + age
...
We are starting with every covariate we think matters, that we think explains the Y.
...
summary(m1)
```
Zombies killed not significnat so we can drop that
```{r}
drop1(m1, test = "F")
```
...
summary(m2)
```
Height explained by asge and weight, just like forward selection showed. This may not always the case though.
...
Finally, there’s a very helpful model selection package called AICcmodavg. You may have noticed the extra ‘c’ in AICc; this is a corrected version of AIC that can account for small sample sizes (helpful for most of us in Anthropology). *SEE MODULE FOR FORMULA*
