...
#### General Process
Clean the data obtained from the training dataset
Form a proper algorithm for building a prediction model
Train the machine and model to understand the pattern of the dataset/project
Predict your results more efficiently and with higher accuracy
...
MSE=(1/n)Σni=1(actual(y)i−predicted(y)i)2
...
RMSE=√(1/n)Σni=1(actual(y)i−predicted(y)i)2
...
RMSLE=√(1/n)Σni=1(log(actual(y)i−predicted(y)i))2
...
MAE=(1/n)Σni=1(|actual(y)i−predicted(y)i|)2
...
LL=−(1/n)Σni=1(actual(y)i)(log(prob(y)i))+(1−(actual(y)i))(log(1−prob(y)i))

The negative average of the log of the error. Error in this case compares the actual classification (from data not in training sample) to the model’s predicted probabilities (prob(y)i is probability of model assigning to class 1 and 1−prob(y)i prob of assigning to class 0). Luckily, there’s a handy function for calculating log loss. Unfortunately, it’s not in {caret}, but the package {ModelMetrics} (which loading masks confusionMatrix). An optimal model minimized the log loss function; a log loss of 0 would be a perferct model.
